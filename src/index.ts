...
export function stableMeta(payload: Json): Row {
  const meta: Row = {}
...
export function apply(ctx: Context): void {
   ...
   */
  async function waitStableJson(args: Json, exec: { signal: AbortSignal }): Promise<{ dataUrl: string; meta: Row }> {
    await ensureRuntime(exec)
    try {
      const resp = await server.request({ op: 'wait_stable', ...args }, exec)
      return { dataUrl: String(resp.data_url ?? ''), meta: stableMeta(resp) }
    } catch {
      const cli = buildCaptureCli(args)
      ...
      if (args.timeout) cli.push('--timeout', String(args.timeout))
      if (args.threshold != null) cli.push('--threshold', String(args.threshold))
      if (args.stable_samples != null) cli.push('--stable-samples', String(args.stable_samples))
      const out = await runCliCapture(cli, exec)
      const info = JSON.parse(out) as Row
      return { dataUrl: String(info.data_url ?? ''), meta: stableMeta(info) }
    }
  }
    ...
    defineTool({
      ...
      output: {
        ...
        render: (_args, value) => {
          ...
          const boxes = (value as any).diff_boxes
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        '轮询截图，直到画面**连续若干次不再变化**才返回（等加载完成、等列表渲染完、等动画停下）。' +
      ...
      output: {
        ...
        render: (_args, value) => {
          const stable = (value as any).stable === true
          const quiet = Number((value as any).stable_for ?? 0)
          const elapsed = String((value as any).elapsed_ms ?? '?')
          const samples = String((value as any).samples ?? '?')
          const head = stable
            ? `画面已稳定（连续 ${quiet} 次无变化，${elapsed}ms / ${samples} 次采样）`
            : `超时仍不稳定：最大差异 ${(Number((value as any).max_diff_ratio ?? 0) * 100).toFixed(2)}% 像素（${elapsed}ms / ${samples} 次采样），画面一直在变`
          return [
            { type: 'image', attachment: (value as any).ref as ImageAttachmentRef },
            { type: 'text', text: head },
          ] as any
      ...
      async execute(args, exec) {
        ...
        if (args.region) req.region = String(args.region)
        if (args.interval) req.interval = Number(args.interval)
        if (args.stable_samples != null) req.stable_samples = Number(args.stable_samples)
        if (args.timeout) req.timeout = Number(args.timeout)
        if (args.threshold != null) req.threshold = Number(args.threshold)
        const { dataUrl, meta } = await waitStableJson(req, exec)
        const { data, mediaType, ext } = parseDataUrl(dataUrl)
