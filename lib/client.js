...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * 截图落成草稿图并塞进输入框附件栏。
     *
     * 三步都可能失败，且**每一步都要让用户看得见**：抓屏失败（非取消）、草稿图建不出来
     * （服务不可达 / 类型被拒）、输入框正忙拒绝入轨。上游把三种情况都写成了静默返回，
     * 结果就是「点了没反应」——这正是它一直没被发现的 bug。
    ...
    function ScreenshotButton(props) {
      ...
      const button = react.createElement(
        'button',
        {
          ...
          title: props.t('button.tooltip'),
          'aria-label': props.t('button.aria'),
          onMouseDown: (event) => {
            event.preventDefault()
          },
          onClick: () => {
            void insertScreenshot(props)
          },
        },
        react.createElement(
          'svg',
