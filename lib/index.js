 ...
 */
async function ensureTargetFront(exec, at) {
    if (lastSeeHandle == null)
        return;
    const handle = lastSeeHandle;
    const args = ['--ensure-front', String(handle), '--unblock'];
    if (at)
        args.push('--at', String(Math.round(at.x)), String(Math.round(at.y)));
...
export function apply(ctx) {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            if (target != null)
                lastSeeHandle = target; // 刚置前的窗口就是接下来的操作目标
            return { ok: true };
        },
    }));
}
